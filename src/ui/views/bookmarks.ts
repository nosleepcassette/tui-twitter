...
export class BookmarksView implements TuitterView {
    ...
    if (this.items.length === 0) {
      return {
        ...
        content: Box(
          ...
          Text({ content: "No saved bookmarks yet.", fg: theme.textMuted }),
        ),
      };
    }
  ...
  private async loadMore(): Promise<void> {
    ...
    try {
      const page = await getBookmarks(this.ctx.client, this.ctx.me.id, {
        paginationToken: this.nextToken,
        maxResults: 20,
      });
      this.items = [...this.items, ...page.items];
      this.nextToken = page.nextToken;
      this.ctx.setStatus(`Loaded ${page.items.length} bookmarks.`);
    } catch (error) {
      this.ctx.setStatus(`Bookmarks request failed: ${this.formatError(error)}`);
    } finally {
      this.loading = false;
    }
  }
